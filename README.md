...
Point Storvi at a book's page and it imports the whole catalogue, resuming where it
stopped. Step-by-step with screenshots: **[docs/whole-book-import.md](docs/whole-book-import.md)**.

### AI selector generation

Content selectors can be generated by a model instead of picked by hand. Three
backends exist, and auto-detection takes the first that is configured — **Ollama,
then Gemini, then Mistral** — so a self-hosted model is preferred over a billed
API. The endpoint returns `503` when none is.

|Backend|Setting|Notes|
|---|---|---|
|Ollama (local)|`OLLAMA_URL`, `OLLAMA_MODEL`|No page content leaves the machine, no API key. Default model `qwen2.5-coder:7b`.|
|Gemini|`GEMINI_API_KEY`|Also used by the translation feature.|
|Mistral|`MISTRAL_API_KEY`, `MISTRAL_MODEL`|Chat completions in JSON mode; default model `mistral-large-latest`.|

`AI_PROVIDER` forces one (`ollama`, `gemini` or `mistral`). That is the fix when
detection picks a backend this host cannot reach: `docker-compose.yml` sets
`OLLAMA_URL` for the whole stack, so on a machine without an NVIDIA runtime the
`ollama` sidecar cannot start and detection would keep choosing a dead endpoint.

```bash
# use Mistral on a host where the ollama service cannot run
MISTRAL_API_KEY=... AI_PROVIDER=mistral docker compose up -d --no-deps storvi
```

With `OLLAMA_URL` set, generation runs **locally** — no page content leaves the
...
- **Requests are serialized.** Two concurrent generations on one 8 GB card would
  thrash, so the server queues them. Hosted backends are not queued that way —
  they only share the general `MAX_AI_CONCURRENCY` bound.
